...
CLASS_DECORATOR_REPLACEMENT_ATTACK = PREFIX + r"""

class _RealMutator:
    def __call__(self, carrier, **values):
...
def _run_attestor(project_root, pattern, challenge_fd, receipt_fd, ready_fd):
    ...
    carrier = {}
    mutator = _Mutator
    mutator(carrier, settings=options)
""" + SUFFIX


DYNAMIC_NEW_REBIND_ATTACK = PREFIX + r"""
...
def _replacement_new(cls, carrier):
    return _RealMutator(carrier)


_Mutator.__new__ = staticmethod(_replacement_new)
...
COMPREHENSION_CARRIED_CALLABLE_ATTACK = PREFIX + r"""
...
def _run_attestor(project_root, pattern, challenge_fd, receipt_fd, ready_fd):
    ...
    options = {}
    carrier = {}
    mutator = next(item for item in [_Mutator(carrier)])
    mutator(settings=options)
""" + SUFFIX


class TerminalAttestationConstructorDynamicCallableTransportTests(unittest.TestCase):
    ...
    def test_class_decorator_replacement_cannot_hide_mapping_transport(self) -> None:
        self._assert_rejected(CLASS_DECORATOR_REPLACEMENT_ATTACK)

    def test_dynamic_new_rebind_cannot_hide_mapping_transport(self) -> None:
        self._assert_rejected(DYNAMIC_NEW_REBIND_ATTACK)

    def test_comprehension_carried_callable_cannot_hide_mapping_transport(self) -> None:
        self._assert_rejected(COMPREHENSION_CARRIED_CALLABLE_ATTACK)
