...
DICT_SETITEM_REBIND = PREFIX + r'''
def _rebind():
    namespace = _replacement.__globals__
    dict.__setitem__(namespace, "_Mutator", _replacement)
''' + ATTESTOR


DICT_UPDATE_REBIND = PREFIX + r'''
def _rebind():
    namespace = _replacement.__globals__
    dict.update(namespace, {"_Mutator": _replacement})
''' + ATTESTOR


REFLECTED_DICT_SETITEM_REBIND = PREFIX + r'''
def _rebind():
    namespace = _replacement.__globals__
    setter = getattr(dict, "__setitem__")
    setter(namespace, "_Mutator", _replacement)
''' + ATTESTOR


class TerminalAttestationFunctionGlobalsDictMethodAuthorityTests(unittest.TestCase):
    ...
    def test_dict_setitem_rebind_is_rejected(self) -> None:
        self._assert_replacement_rejected(DICT_SETITEM_REBIND)

    def test_dict_update_rebind_is_rejected(self) -> None:
        self._assert_replacement_rejected(DICT_UPDATE_REBIND)

    def test_reflected_dict_setitem_rebind_is_rejected(self) -> None:
        self._assert_replacement_rejected(REFLECTED_DICT_SETITEM_REBIND)
