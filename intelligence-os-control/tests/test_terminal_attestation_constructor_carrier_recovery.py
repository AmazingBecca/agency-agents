...
def _run_attestor(project_root, pattern, challenge_fd, receipt_fd, ready_fd):
    ...
    holder = _Holder(_reflection_factory())
    helper_name = "".join(("_spawn", "_shadow"))
    holder.get_resolver()(sys.modules[__name__], helper_name)()
    return_code = candidate.wait()
    _write_all(ready_fd, b"candidate-exited\n")
    token = os.read(challenge_fd, 32)
    _write_all(receipt_fd, token)
...
def _run_attestor(project_root, pattern, challenge_fd, receipt_fd, ready_fd):
    ...
    holder = _Holder(_reflection_factory())
    helper_name = "".join(("_spawn", "_shadow"))
    holder.exposed_resolver(sys.modules[__name__], helper_name)()
    return_code = candidate.wait()
    _write_all(ready_fd, b"candidate-exited\n")
    token = os.read(challenge_fd, 32)
    _write_all(receipt_fd, token)
...
class TerminalAttestationConstructorCarrierRecoveryTests(unittest.TestCase):
    ...
    def _assert_rejected_with_authority(self, source: str, authority: str) -> None:
        ...
        self.assertIn(authority, details)
    ...
    def test_positional_constructor_field_cannot_launder_helper_authority(self) -> None:
        self._assert_rejected_with_authority(ATTACK, "holder.resolver")

    def test_property_descriptor_cannot_launder_constructor_authority(self) -> None:
        self._assert_rejected_with_authority(PROPERTY_ATTACK, "holder.resolver")

    def test_zero_argument_accessor_cannot_launder_constructor_authority(self) -> None:
        self._assert_rejected_with_authority(ACCESSOR_ATTACK, "holder.get_resolver")

    def test_chained_property_descriptor_cannot_launder_constructor_authority(self) -> None:
        self._assert_rejected_with_authority(
            CHAINED_PROPERTY_ATTACK,
            "holder.exposed_resolver",
        )
