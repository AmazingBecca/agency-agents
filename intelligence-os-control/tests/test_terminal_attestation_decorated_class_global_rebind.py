...
TRANSITIVE_GLOBAL_REBIND = PREFIX + r"""
def _rebind():
    global _Mutator
    _Mutator = _replacement
...
UNREACHABLE_GLOBAL_REBIND = PREFIX + r"""
def _unused_rebind():
    global _Mutator
    _Mutator = _replacement
...
class TerminalAttestationDecoratedClassGlobalRebindTests(unittest.TestCase):
    ...
    def test_reachable_helper_global_rebind_is_rejected(self) -> None:
        self._assert_rejected(DIRECT_GLOBAL_REBIND)

    def test_transitive_reachable_global_rebind_is_rejected(self) -> None:
        self._assert_rejected(TRANSITIVE_GLOBAL_REBIND)

    def test_branch_scoped_global_rebind_is_rejected(self) -> None:
        self._assert_rejected(BRANCH_GLOBAL_REBIND)

    def test_unreachable_global_rebind_does_not_poison_secure_attestor(self) -> None:
        self.assertTrue(self._verify(UNREACHABLE_GLOBAL_REBIND)["passed"])
